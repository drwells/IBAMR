...
main(int argc, char** argv)
{
    ...
    if (timestep)
    {
        ...
        for (int i = starting_step; i < num_timesteps; ++i)
        {
            if (test_h5part)
            {
                marker_points.writeH5Part("markers-" + std::to_string(i) + ".h5part");
            }
        ...
        if (test_h5part)
        {
            const std::string last_file_name = "markers-" + std::to_string(num_timesteps + 1) + ".h5part";
            marker_points.writeH5Part(last_file_name);
