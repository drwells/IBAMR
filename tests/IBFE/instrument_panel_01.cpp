...
main(int argc, char** argv)
{
    ...
    TimerManager::createManager(nullptr);
    {
        ...
        auto do_instrument_panel = [&](const int data_time)
        {
            ...
            instrument_panel.initializeHierarchyDependentData(&*ib_method_ops, patch_hierarchy);
            instrument_panel.readInstrumentData(u_copy_idx, p_copy_idx, patch_hierarchy, 1.0);
            ...
            for (auto el_it = mesh.active_local_elements_begin(); el_it != end_el; ++el_it)
            {
                ...
                for (unsigned int side_n = 0; side_n < elem.n_sides(); ++side_n)
                {
                    ...
                    if (side_ptr->true_centroid()(2) == data_time + 0.0)
                    {
                        fe->reinit(&elem, side_n);
                        // hard-codes in the flux as the z velocity * -1
                        for (std::size_t qp = 0; qp < JxW.size(); ++qp)
                            bottom_flux += -1.0 * JxW[qp] * exact_solution.value(q_points[qp], 2);
                    }
                    else if (side_ptr->true_centroid()(2) == data_time + 1.0)
                    {
                        fe->reinit(&elem, side_n);
                        for (std::size_t qp = 0; qp < JxW.size(); ++qp)
                            top_flux += 1.0 * JxW[qp] * exact_solution.value(q_points[qp], 2);
                    }
                }
            }
            ...
            bottom_flux = IBTK_MPI::sumReduction(bottom_flux);
            top_flux = IBTK_MPI::sumReduction(top_flux);

            plog << "bottom flux = " << bottom_flux << std::endl;
            plog << "top flux = " << top_flux << std::endl;
        };
        ...
        // second test: translate the mesh
        {
            ...
            NumericVector<double>& solution = *dX_system.solution;
            solution.add(1.0);
        ...
        // third test: dilate the mesh
        {
            ...
            for (auto node_it = mesh.nodes_begin(); node_it != end_node_it; ++node_it)
            {
                ...
                solution.set(component_dofs[0], new_x - current_x);

                // translate z
                IBTK::get_nodal_dof_indices(dX_system.get_dof_map(), &node, 2, component_dofs);
                TBOX_ASSERT(component_dofs.size() == 1);
                solution.set(component_dofs[0], 2.0);
            }
