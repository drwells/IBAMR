...
MarkerPatchHierarchy::putToDatabase(SAMRAI::tbox::Pointer<SAMRAI::tbox::Database> db)
{
    ...
    for (const auto& level_marker_patches : d_marker_patches)
    {
        ...
        {
            const std::string key_prefix = d_object_name + "::" + std::to_string(marker_patch_num);
            put_marker_patch(marker_patch, key_prefix);
            ++marker_patch_num;
        }
    }

    put_marker_patch(d_markers_outside_domain, d_object_name + "::markers_outside_domain");
}

void
MarkerPatchHierarchy::writeH5Part(const std::string& filename, const bool write_velocities) const
{
...
MarkerPatchHierarchy::getFromDatabase(SAMRAI::tbox::Pointer<SAMRAI::tbox::Database> db)
{
    ...
    get_marker_patch(d_markers_outside_domain, d_object_name + "::markers_outside_domain");
...
void
MarkerPatchHierarchy::midpointStep(const double dt, const int u_half_idx, const int u_new_idx, const std::string& kernel)
{
    const auto rank = IBTK_MPI::getRank();
    for (int ln = d_hierarchy->getFinestLevelNumber(); ln >= 0; --ln)
    {
